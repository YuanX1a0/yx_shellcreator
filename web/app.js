...
function buildTypeLabel(type) {
    return ({ floor: '地板', wall: '墙面', door: '门', stairs: '楼梯', railing: '栏杆' })[type] || '建筑部件';
}
...
function updateSceneChrome() {
    ...
    focusHint.textContent = isEmptyBuilder()
        ? '斜俯视建造 · WASD 平移 · E/Q 或滚轮缩放 · 按住右键旋转 · F2 返回面板'
        : '自由视角 · 左键选择/拖拽坐标轴 · 按住右键环视 · F2 返回面板';
    ...
    if (ui.construction.active) {
        document.getElementById('buildToolName').textContent = ui.construction.label || '建筑部件';
        document.getElementById('buildToolMeta').textContent = `${buildTypeLabel(ui.construction.buildType)} · 旋转 ${Math.round(Number(ui.construction.rotation) || 0)}° · 步进 ${Math.round(Number(ui.construction.rotationStep) || 90)}°`;
    }
}
...
function renderManager() {
    ...
    const houses = ui.houses.filter(house => `${house.label} ${house.slug} ${(presetById(house.presetId) || {}).label || ''}`.toLowerCase().includes(query));
    houseList.innerHTML = houses.map((house) => {
        ...
        return `
            <article class="house-row" data-house-id="${escapeHtml(house.id)}">
                <div class="house-name"><h3>${escapeHtml(house.label)}${active ? '<span class="active-state">当前</span>' : ''}</h3><code>${escapeHtml(house.slug)}</code></div>
                <div class="house-type">${escapeHtml(preset ? preset.label : house.presetId)}<small>${1 + (house.accessPoints || []).length} 个出入口</small></div>
                <div class="house-actions">
                    <button class="button" data-house-action="enter">进入</button>
                    <button class="button ghost" data-house-action="edit">编辑</button>
                    <button class="button ghost danger-text" data-house-action="delete">删除</button>
                </div>
            </article>`;
    }).join('') || `<div class="empty-list"><strong>${ui.houses.length ? '没有匹配的房屋' : '还没有房屋'}</strong><small>${ui.houses.length ? '换一个关键词试试。' : '站到入口位置，点击右上角「新建房屋」。'}</small></div>`;
    renderAccessPoints();
}

function renderBuilder() {
    document.getElementById('builderHouseName').textContent = ui.currentHouse ? ui.currentHouse.label : '室内建造';
    document.getElementById('emptyBuildBanner').classList.toggle('hidden', !isEmptyBuilder());
    document.getElementById('releaseFocus').textContent = isEmptyBuilder() ? '进入斜俯视建造 / F2' : '进入自由视角 / F2';
...
function renderCatalog() {
    ...
    document.getElementById('catalogResultCount').textContent = `${result.total} 项`;
    ...
    catalogGrid.innerHTML = result.items.map((item) => {
        const image = item.image
            ...
            : `<span class="catalog-no-preview"><b>${escapeHtml(categoryLabel(item.category))}</b><span>无缩略图</span></span>`;
        const buildBadge = item.door ? '<em class="build-type door">可推开门扇</em>' : item.buildType
            ? `<em class="build-type">${item.buildType === 'door' ? '门洞 · 另配门扇' : escapeHtml(buildTypeLabel(item.buildType))}</em>`
            : !item.image ? `<em class="build-type">${escapeHtml(categoryLabel(item.category))} · 无缩略图</em>` : '';
        return `
            <button class="catalog-item ${!item.image ? 'no-image' : ''} ${item.category === 'construction' ? 'construction' : ''}" data-item-id="${escapeHtml(item.id)}" data-model="${escapeHtml(item.model)}" title="${escapeHtml(item.model)} · ${escapeHtml(item.searchTags || item.sourceLabel || '')}${item.available === false ? ' · ' + escapeHtml(item.unavailableReason || '当前客户端未注册此模型') : ''}" ${item.available === false ? 'disabled' : ''}>
                <span class="catalog-thumb">${image}</span>
                ${buildBadge}
                <strong>${escapeHtml(item.label || item.model)}</strong>
                ${item.originalLabel ? `<small>${escapeHtml(item.originalLabel)}</small>` : ''}
                <small>${item.available === false ? '当前客户端不可用' : escapeHtml(item.model)}</small>
            </button>`;
    }).join('') || '<div class="empty-list"><span>⌕</span><strong>没有匹配的模型</strong><small>尝试更换分类或搜索关键词。</small></div>';
}

function objectLabel(item) {
    const entry = (ui.catalog.items || []).find(c => c.model === item.model);
    return entry ? entry.label : item.model;
}
...
function renderObjects() {
    ...
    objectList.innerHTML = ui.objects.map((item, index) => `
        <button class="object-row ${ui.selectedId === item.id ? 'active' : ''} ${item.hidden ? 'hidden-native' : ''}"
            ...
            <span class="object-name">${escapeHtml(objectLabel(item))}${item.sourceKind === 'native' ? '<small>原生</small>' : ''}${item.isDoor ? '<small>门扇</small>' : ''}</span>
            ${item.hidden ? '<em>恢复</em>' : ''}
        </button>`).join('') || '<p class="muted object-empty">暂无已放置物件</p>';
}
...
function renderSelection() {
    ...
    if (!item) return;
    document.getElementById('selectedLabel').textContent = objectLabel(item);
    ...
    document.getElementById('selectedSource').textContent = item.sourceKind === 'native'
        ? '原生室内物件 · 删除后会隐藏，可从列表恢复'
        : item.isDoor ? '门扇 · 不关联传送点' : '拖动场景中的坐标轴调整位置';
}
...
function resetForm() {
    ...
    document.getElementById('formTitle').textContent = '创建房屋';
    document.getElementById('submitHouse').textContent = '在当前位置创建';
...
function editHouse(house) {
    ...
    document.getElementById('formTitle').textContent = '编辑房屋';
    document.getElementById('submitHouse').textContent = '保存修改';
...
function renderAccessPoints() {
    ...
    const renderPoints = (house, indoor) => ((house && house.accessPoints) || []).map(point => `
        <div class="access-row" data-point-id="${escapeHtml(point.id)}" data-access-house="${escapeHtml(house.id)}">
            <strong>${escapeHtml(point.label)}</strong><p>${indoor ? '屋内' : '外部'}：${escapeHtml(coordinates(indoor ? point.exit : point.entrance))}</p>
            <div class="toolbar-row"><button class="button" data-access-action="${indoor ? 'exit' : 'entrance'}" ${ui.accessBusy || (!indoor && inside) ? 'disabled' : ''}>${indoor ? '屋内位置设在此处' : '外部入口移到此处'}</button><button class="button ghost danger-text" data-access-action="delete" ${ui.accessBusy ? 'disabled' : ''}>删除</button></div>
        </div>`).join('') || '<p class="muted">暂无附加出入口，默认出入口保持不变。</p>';
...
async function changeAccessPoint(data) {
    ...
    if (!result.ok) { ui.accessBusy = false; renderAccessPoints(); toast('出入口提交失败。', 'error'); }
}
...
document.getElementById('doorHoldOpen').addEventListener('change', async () => {
    ...
    if (!result.ok) { renderSelection(); toast('门状态提交失败。', 'error'); }
});
document.getElementById('addAccessPoint').addEventListener('click', () => {
    ...
    if (!label) { document.getElementById('accessPointLabel').focus(); toast('先填写出入口名称。'); return; }
...
for (const id of ['managerAccessList','houseAccessList']) document.getElementById(id).addEventListener('click', event => {
    ...
    if (button.dataset.accessAction === 'delete') confirmTwice(`access:${row.dataset.pointId}`, '即将删除这个出入口，不删除门模型。', action);
    else action();
});
...
function confirmTwice(key, message, action) {
    ...
    toast(`${message} 请在 4 秒内再次点击删除确认。`, 'warning');
}
...
document.getElementById('deleteObject').addEventListener('click', () => {
    const item = ui.objects.find((entry) => entry.id === ui.selectedId);
    if (item) confirmTwice(`object:${item.id}`, `即将删除物件 ${item.model}。`, () => post('deleteObject'));
});
...
houseForm.addEventListener('submit', (event) => {
    ...
    if (ui.houseBusy) return;
    if (ui.importError) { toast('请重新读取有效文件，或点击取消导入。', 'error'); return; }
    ...
    post(operation, payload).then(result => { if (!result.ok) { setHouseBusy(false); toast('提交失败，请检查数据后重试。','error'); } });
});
...
houseList.addEventListener('click', (event) => {
    ...
    if (action.dataset.houseAction === 'delete') {
        confirmTwice(`house:${house.id}`, `即将删除“${house.label}”及其全部建筑物。`, () => post('deleteHouse', { houseId: house.id }));
    }
});
...
window.addEventListener('message', (event) => {
    ...
    if (data.action === 'open') {
        ...
        document.getElementById('previewDoor').textContent = data.active ? '结束试开' : '试开门';
        document.getElementById('doorPreviewStatus').textContent = data.active ? '预览中 · 不保存' : '接触推开';
        ...
        document.getElementById('houseExportLocation').textContent = data.saved
            ? `服务器资源目录：yx_shellcreator/${data.filename}` : '服务器未能保存文件，请复制 JSON 后自行保存。';
...
function clearHouseImport() {
    ...
    document.getElementById('houseImportSummary').textContent = '不选择文件则按所选模板创建空房屋。';
    presetSelect.disabled = false;
    document.getElementById('shellModel').disabled = false;
    if (!document.getElementById('editingHouseId').value) document.getElementById('submitHouse').textContent = '在当前位置创建';
}

function readHouseImport(text) {
    ui.importDocument = null; ui.importError = true;
    try {
        if (new Blob([text]).size > 2*1024*1024-4096) throw new Error('文件不能超过 2 MiB。');
        ...
        if (!data || data.format !== 'yx_shellcreator.house' || data.version !== 1 || data.coordinateMode !== 'relative'
            || !data.house || !Array.isArray(data.objects)) throw new Error('不是支持的房屋数据文件（版本 1）。');
        if (data.objects.length > (ui.config.maxObjectsPerHouse || 800)) throw new Error('文件物件数量超过本服上限。');
        const preset = presetById(data.house.presetId);
        if (!preset || preset.type !== data.house.presetType) throw new Error('本服没有对应室内模板，或模板类型不同。');
        ...
        if (!document.getElementById('houseLabel').value) document.getElementById('houseLabel').value = String(data.house.label || '导入房屋').slice(0,80);
        if (!document.getElementById('houseSlug').value) document.getElementById('houseSlug').value = `import_${Date.now().toString(36)}`;
        updatePresetHelp();
        document.getElementById('submitHouse').textContent = '导入并创建新房屋';
        document.getElementById('houseImportSummary').textContent = `${data.objects.length} 个物件 · ${preset.label}。${preset.type === 'world'
            ? '保留原地图室内坐标；新入口为人物当前位置。' : '按本服模板原点还原布局；新入口为人物当前位置。'} 模型资源需另行安装。`;
    } catch (error) {
        const message = error instanceof SyntaxError ? 'JSON 格式有误，请检查引号、逗号和括号是否完整。' : error.message;
        document.getElementById('houseImportSummary').textContent = `导入未就绪：${message}`;
        toast(message, 'error');
    }
}
...
document.getElementById('houseImportFile').addEventListener('change', async event => {
    ...
    ui.importDocument = null; ui.importError = true;
    document.getElementById('houseImportSummary').textContent = '正在读取文件，请稍候…';
    if (file.size > 2*1024*1024-4096) {
        document.getElementById('houseImportSummary').textContent = '导入未就绪：文件不能超过 2 MiB。';
        toast('文件不能超过 2 MiB。', 'error');
        return;
    }
    try { const text = await file.text(); if (token === importReadToken) readHouseImport(text); }
    catch { if (token === importReadToken) {
        document.getElementById('houseImportSummary').textContent = '文件读取失败，可改用粘贴 JSON。';
        toast('文件读取失败，可改用粘贴 JSON。','error');
    } }
});
...
document.getElementById('saveHouseEnvironment').addEventListener('click', async event => {
    ...
    if (fixed && !/^([01]\d|2[0-3]):[0-5]\d$/.test(time)) { toast('请填写 00:00 至 23:59 的时间。','error'); return; }
...
document.getElementById('copyHouseExport').addEventListener('click', async () => {
    ...
    try { await navigator.clipboard.writeText(field.value); toast('完整 JSON 已复制。','success'); }
    catch { field.focus(); field.select(); toast(document.execCommand('copy') ? '完整 JSON 已复制。' : '已选中全部数据，请按 Ctrl+C。'); }
});
document.getElementById('downloadHouseExport').addEventListener('click', () => {
    ...
    toast('若客户端未出现下载，请从服务器 exports 目录取文件或复制 JSON。');
});
